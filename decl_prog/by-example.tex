...
\]
% XXX Why include "n >= 0"? It is implied by "0 =< m =< n".
From these formulas,
% XXX Although we have used pseudo-code,
...
\]
+% XXX Mark: why the switch from app as predicate to app as function?
...
% XXX Yes, they could. The characters could be invalid,
% just as an "ASCII" char could be invalid by having the top bit set.
% XXX The "all NUL-terminated bytes sequences are valid" property
% IS true for the ISO-8859-1 char encoding.
...
% XXX This issue is specific to UTF-8;
% it doesn't arise for UTF-32, for example.
...
% XXX Do we talk about inadmissible calls somewhere?
% If yes, then we should qualify the above.
\item
Bug localization.
...
% XXX That is its only job.

% XXX "digits" would a better name for this type
\begin{figure}[htb]
\begin{verbatim}
