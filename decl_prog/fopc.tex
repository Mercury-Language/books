...
% XXX This quantification seems strange. Why not say
% A set of function symbols
% \sym{f}, \sym{g}, \ldots,
% each of which has a non-negative arity,
% which we denote as e.g.
% \sym{f/2}, \sym{g/1}, \ldots
\item
For $n \geqslant 0$,
a set of predicate symbols with arity $n$:
\sym{p/2}, \sym{q/3}, \ldots
% XXX I would propose the same wording here.
\end{enumerate}
...
% XXX Which is why we should use "user-defined", instead of "non-logical",
% from now on.
...
% XXX Then why not include $a$, $b$ in point 1 above?
% XXX Should we write that we use "letters near the ends of the alphabet"
% for variables?
...
% XXX Which is?
% usually negation bind tightest, followed by conjunction, followed by
% disjunction, but where do implications and quantifiers fit in?
...
We will use names such as \sym{f} and \sym{g} to stand for arbitrary functions,
% XXX If you mean to include data constructors in this,
% then this should say "function symbols" instead of "arbitrary functions".
and names such as \sym{p} and \sym{q} to stand for predicates.
We will sometimes refer to function symbols with arity zero
as \emph{constants\label{gi:constant}},
and use names such as \sym{a}, \sym{b} and \sym{c}
to stand for arbitrary constants.
% XXX This clashes with the use of $a$, $b$ etc as variables, mentioned above.
% They differ in font, but (1) some readers won't notice that,
% and (2) syntactic constants also need their semantic equivalents.
...
but any goal of the form \co{not Goal} can be transparently replaced
...
However, there are some kinds of goals
...
% % XXX Readers don't yet know what those "other goals" are.
